...
elif "[SLOT_CODEX_VERDICT" in reason:
    marker_match = re.search(r"/tmp/codex-app-(plan|code|qa)-review-(\d+)\.txt", reason)
    helper = "/Users/rajiv/.claude/scripts/slot-report-codex-verdict.sh"
    command = f"{helper} --pr <PR> --kind code --next \"PM adjudication\""
    if marker_match:
        kind, ident = marker_match.groups()
        if kind == "plan":
            command = f"{helper} --issue {ident} --kind plan --next \"PM adjudication\""
        else:
            command = f"{helper} --pr {ident} --kind {kind} --next \"PM adjudication\""
    reason += (
        "\n\nRequired next action: run the deterministic Codex marker reporter:\n"
        f"{command}\n"
        "It reads the marker and sends PM the exact MARKER_PROVENANCE, TIMESTAMP, "
        "COMPANION_VERDICT, and FINAL_REVIEWER_VERDICT fields. Do not hand-compose "
        "abbreviated fields like COMPANION= or FINAL=."
    )
else:
    reason += (
