...
function payloadReferencesIssueExactly(value, issueNumber) {
...
function isCodexReviewerAgentEvent(eventType, toolName, payload, issueNumber) {
    ...
    try {
        ...
        if (!input || input.subagent_type !== "codex-code-reviewer")
            return false;
...
export function normalizeAssignmentTuple(value) {
    ...
    const issue = value.issue === null
        ...
        : Number.isInteger(value.issue) && Number(value.issue) > 0
            ? Number(value.issue)
            : null;
    if (value.issue !== null && issue === null)
        return null;
    ...
    const headSha = value.head_sha === null
        ...
        : typeof value.head_sha === "string" && /^[0-9a-f]{40}$/i.test(value.head_sha)
            ? value.head_sha
            : null;
    if (value.head_sha !== null && headSha === null)
        return null;
    const workKind = value.work_kind === null
        ...
        : typeof value.work_kind === "string" && ASSIGNMENT_WORK_KINDS.has(value.work_kind.trim())
            ...
            : null;
    if (value.work_kind !== null && workKind === null)
        return null;
    const handoffId = value.handoff_id === null
        ...
        : typeof value.handoff_id === "string" && value.handoff_id.trim()
            ...
            : null;
    if (value.handoff_id !== null && handoffId === null)
        return null;
    const claimedAt = value.claimed_at === null
        ...
        : typeof value.claimed_at === "string" && value.claimed_at.trim()
            ...
            : null;
    if (value.claimed_at !== null && claimedAt === null)
        return null;
    if ((workKind === null) !== (handoffId === null))
        return null;
    if (pr !== null && (branchIdentity.branchRef === null || headSha === null))
        return null;
...
export function normalizedFamily2ReleaseBody(request) {
    const tuple = normalizeAssignmentTuple(request.expected_tuple);
    if (!tuple)
        throw new Error("Family-2 release tuple is invalid");
    if (typeof request.effect_id !== "string"
...
export function computeFamily2ReleaseDigest(request) {
    return createHash("sha256")
        .update(JSON.stringify(normalizedFamily2ReleaseBody(request)))
        .digest("hex");
}
export function slotAssignmentTuple(slot) {
    if (!slot.repository_id)
        return null;
    return {
...
export function assignmentTupleMatches(left, right) {
    if (!left || !right)
        return false;
...
export class MoPDatabase {
    ...
    `);
        ...
        const columns = this.db.prepare("PRAGMA table_info(slots)").all();
    ...
    `);
        for (const i of devSlots(this.config.slotCount)) {
            ...
            if (name)
                fillMissingSlotName.run(name, i);
        }
    }
    migrateAssignmentIdentity() {
    ...
    `).all();
        this.db.transaction(() => {
            for (const row of legacyRows) {
                ...
                if (!branchIdentity) {
                    throw new Error(`cannot migrate slot ${row.slot}: invalid legacy branch ${JSON.stringify(row.branch)}`);
                }
          ...
          `).run(branchIdentity.branch, branchIdentity.branchRef, row.slot);
                ...
                if (row.occupied === 1 && !row.repository_id) {
                    if (!legacyRepositoryId) {
                        throw new Error("MOP_LEGACY_REPOSITORY_ID is required to migrate occupied legacy slots");
                    }
                    this.db.prepare(`
    ...
    configuredLegacyRepositoryId() {
        if (this.config.legacyRepositoryId === null)
            return null;
    ...
    // ─── Event Log ───────────────────────────────────────────
    logEvent(slot, eventType, hookType, toolName, payload) {
    ...
    getEvents(slot, limit = 50, eventType) {
     ...
     */
    pruneEvents(maxRows = 200_000, maxAgeDays = 14) {
     ...
     */
    getRecentEscalation(slot, issueNum, sinceIso) {
        return this.db
            .prepare(`SELECT id, timestamp FROM events
         WHERE slot = ? AND event_type = 'escalation'
           AND timestamp > ?
           AND payload LIKE ?
         ORDER BY id DESC LIMIT 1`)
            .get(slot, sinceIso, `%"issue":${issueNum}%`);
    }
    // ─── Slot State ──────────────────────────────────────────
    getSlot(slot) {
        const row = this.db
    ...
    updateAssignmentState(slot, updates) {
    ...
    updateSlotFields(slot, updates, allowedFields) {
        const sets = [];
        const values = [];
     ...
     */
    commitNativeRelease(slot, expectedEpoch, expectedTupleInput, effect) {
        ...
        if (effect && (typeof effect.effect_id !== "string"
        ...
        return this.db.transaction(() => {
            if (effect) {
                ...
                if (prior) {
                    const sameTuple = assignmentTupleMatches(normalizeAssignmentTuple(prior.expected_tuple), expectedTuple);
                    const current = this.getSlot(slot);
                    if (prior.request_digest.toLowerCase() !== effect.request_digest.toLowerCase()
                        || prior.slot !== slot
        ...
        `).run(effect.effect_id, computedEffectDigest, slot, expectedEpoch, epoch + 1, "", JSON.stringify({
    ...
    getNativeReleaseEffectReceipt(effectId) {
    ...
    `).get(effectId);
        ...
        let expectedTuple;
        try {
            const parsed = JSON.parse(row.expected_tuple);
    ...
    assignSlot(slot, task, repositoryId, issue, branch, pr = null, headSha = null, expectedEpoch, workKind = null, handoffId = null, requireFree = false) {
        const current = this.getSlot(slot);
        if (!Number.isInteger(expectedEpoch)) {
            return {
                ok: false,
        ...
        if ((workKind !== null && typeof workKind !== "string")
        ...
        return this.db.transaction(() => {
            ...
            if (requireFree && !current.occupied && (current.active_turn_id !== null
        ...
        ORDER BY slot
      `).all(normalizedRepositoryId, slot, normalizedPr, normalizedPr, normalizedIssue, normalizedIssue, branchIdentity.branchRef, branchIdentity.branchRef);
            if (owners.length > 0) {
     ...
     * the next epoch.
     */
    rebindSlot(slot, expectedEpoch, expectedTupleInput, desiredTupleInput, task) {
        ...
        ORDER BY slot
      `).all(desiredTuple.repository_id, slot, desiredTuple.pr, desiredTuple.pr, desiredTuple.issue, desiredTuple.issue, desiredTuple.branch_ref, desiredTuple.branch_ref);
            if (owners.length > 0) {
                return {
                    ok: false,
                    conflict: true,
                    ...
                    owner_conflicts: owners.map((owner) => ({
                        slot: owner.slot,
                        matching_fields: [
                            ...(desiredTuple.issue !== null && owner.issue === desiredTuple.issue ? ["issue"] : []),
                            ...(desiredTuple.pr !== null && owner.pr === desiredTuple.pr ? ["pr"] : []),
                            ...(desiredTuple.branch_ref !== null && owner.branch_ref === desiredTuple.branch_ref ? ["branch_ref"] : []),
                        ],
                    })),
                };
            }
     ...
     * remain unchanged.
     */
    syncSlotCheckout(slot, branch, headSha, expectedEpoch) {
        if (!Number.isInteger(expectedEpoch)) {
            const current = this.getSlot(slot);
            return {
                ok: false,
    ...
    touchMeaningfulWork(slot, _turnId) {
        const now = new Date().toISOString();
        this.updateSlot(slot, {
            last_meaningful_work_at: now,
            idle: false,
    ...
    enqueuePendingPMEvent(slot, eventType, payload = null, enqueuedAt) {
        const stmt = this.db.prepare(`
    ...
    deletePendingPMEvent(slot, eventType, enqueuedAt) {
    ...
    `).all();
        ...
        const isSlotState = (eventType) => slotStateTypes.has(eventType);
        // Priority: lower number = higher priority among slot-state rows only.
        const priorityOf = (eventType) => {
            if (eventType === "slot-blocked")
                return 1;
            if (eventType === "slot-idle")
                return 2;
            if (eventType === "slot-active")
                return 3;
            if (eventType === "check-slot")
                return 4;
     ...
     */
    getPMQueueDeliveryState(slot, eventType, message, enqueuedAt) {
        const rows = this.db.prepare(`
      ...
      ORDER BY id DESC
      LIMIT 1
    `).all(slot, eventType, message, enqueuedAt ?? null, enqueuedAt ?? null);
        if (rows.length === 0)
            return "none";
        const event = rows[0];
        if (event.event_type === "pm_queue_delivery_started")
            return "started";
        if (event.event_type === "pm_queue_delivered")
            return "delivered";
        try {
    ...
    hasPMQueueDelivery(slot, eventType, message, enqueuedAt) {
        return this.getPMQueueDeliveryState(slot, eventType, message, enqueuedAt) === "delivered";
    }
    /** Returns count of rows currently queued. */
    getPendingPMEventCount() {
        const row = this.db.prepare("SELECT COUNT(*) AS n FROM pm_pending_events").get();
        return row?.n ?? 0;
    }
    ...
    createOpsJob(job) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(job.id, job.kind, job.reason, job.status, job.created_at, job.started_at, job.finished_at, job.pid, job.exit_code, job.decision, job.result_reason, job.payload_bytes, job.error, job.stdout_path, job.trace_path);
    }
    ...
    getOpsJob(id) {
        ...
        return row ? normalizeOpsJob(row) : null;
    }
    ...
    getLastVisibleSlotState(slotNum) {
        const row = this.db.prepare(`
     ...
     */
    hasRecentSubagentDispatch(slotNum, windowSec = 60) {
      ...
      LIMIT 20
    `);
        const taskRows = taskStmt.all(slotNum, windowSec);
    ...
    `);
        for (const taskRow of taskRows) {
            if (taskRow.tool_name === "Agent") {
                ...
                try {
                    const payload = JSON.parse(taskRow.payload);
                ...
                const closeRow = agentCloseStmt.get(slotNum, taskRow.timestamp);
                if (!closeRow)
                    return { taskTs: taskRow.timestamp, lastStopTs: null, toolName: "Agent" };
                continue;
            }
            // Foreground Task dispatches are closed by the next Stop.
            const stopRow = stopStmt.get(slotNum, taskRow.timestamp);
            if (!stopRow)
                return { taskTs: taskRow.timestamp, lastStopTs: null, toolName: "Task" };
        }
        return null;
    }
    /**
     ...
     */
    getLastToolFire(slotNum, windowSec = 30) {
      ...
      LIMIT 1
    `);
        const row = stmt.get(slotNum, windowSec);
        if (!row)
            return null;
     ...
     */
    findReviewEvent(issueNumber, windowMinutes = 60) {
      ...
      ORDER BY timestamp DESC
    `);
        const rows = stmt.all(windowMinutes);
        const row = rows.find((candidate) => {
            ...
            return isCodexReviewerAgentEvent(candidate.event_type, candidate.tool_name, candidate.payload, issueNumber);
        });
        if (!row) {
        ...
        if (row.tool_name === "Agent" || row.tool_name === "Task")
            method = "codex-agent";
        else if (payload.includes("codex-app"))
            method = "codex-app";
        else if (payload.includes("zen"))
            method = "zen";
        else if (payload.includes("codex exec"))
            method = "codex-cli";
        else if (payload.includes("codex-companion"))
            method = "codex-plugin";
        else if (payload.includes("codex"))
            method = "codex";
        return {
            found: true,
            method,
