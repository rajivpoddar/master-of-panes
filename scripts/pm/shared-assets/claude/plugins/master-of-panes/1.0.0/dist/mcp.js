...
export async function startMcpServer(config) {
    ...
    server.tool("mop_slot_status", "Read one authoritative numbered-slot state.", { slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT) }, async ({ slot }) => ({
        content: [{ type: "text", text: JSON.stringify(db.getSlot(slot), null, 2) }],
    }));
    server.tool("mop_all_slots", "Read the authoritative numbered-slot registry.", {}, async () => {
    ...
    server.tool("mop_slot_history", "Read recent events for one numbered slot.", {
        slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT),
        limit: z.number().int().min(1).max(200).default(20),
    }, async ({ slot, limit }) => ({
        content: [{ type: "text", text: JSON.stringify(db.getSlotHistory(slot, limit), null, 2) }],
    }));
    server.tool("mop_recent_activity", "Read recent MoP activity without causing an effect.", { minutes: z.number().int().min(1).max(1440).default(60) }, async ({ minutes }) => ({
        content: [{
                type: "text",
                text: JSON.stringify({ events: db.getRecentActivity(minutes) }, null, 2),
            }],
    }));
    server.tool("mop_send_to_slot", "Deliver one explicit literal message to a numbered slot.", {
        slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT),
        ...
        if (state?.dnd) {
            return { isError: true, content: [{ type: "text", text: "slot is DND" }] };
        }
        ...
        return {
            isError: !success,
            content: [{ type: "text", text: JSON.stringify({ success, slot }) }],
        };
    });
    server.tool("mop_release_slot", "Release one exact owning tuple through the native epoch/CAS boundary.", {
        slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT),
        ...
        try {
            const response = await fetch(`http://127.0.0.1:${config.httpPort}/slots/${input.slot}/release`, {
                method: "POST",
                headers: {
            ...
            return {
                isError: !response.ok || payload.success !== true,
    ...
    server.tool("mop_respawn_slot", "Explicitly respawn one idle slot through the native REST operation.", {
        ...
        const response = await fetch(`http://127.0.0.1:${config.httpPort}/slots/${slot}/respawn`, {
            method: "POST",
    ...
    server.tool("mop_set_dnd", "Set or clear DND on a numbered slot.", {
        slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT),
        dnd: z.boolean(),
    }, async ({ slot, dnd }) => {
        ...
        if (dnd && current && !current.occupied) {
            ...
            return { content: [{ type: "text", text: "free slots cannot be DND" }] };
        }
        db.updateSlot(slot, { dnd });
        db.logEvent(slot, dnd ? "dnd_enabled" : "dnd_disabled", null, null, { explicit: true });
        return { content: [{ type: "text", text: JSON.stringify(db.getSlot(slot), null, 2) }] };
    });
    server.tool("mop_capture_output", "Read the current tmux output for one numbered slot.", {
        slot: z.number().int().min(1).max(DEFAULT_DEV_SLOT_COUNT),
