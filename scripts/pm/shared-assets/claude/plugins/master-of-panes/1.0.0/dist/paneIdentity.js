...
export function paneAddress(slot) {
    return `0:0.${slot}`;
}
function shellEscape(value) {
    return `'${value.replace(/'/g, "'\\\\''")}'`;
}
function parsePaneIdentityLine(line) {
...
export function expectedCheckoutPath(slot) {
    if (slot === 0)
        return "/Users/rajiv/Downloads/projects/heydonna-app";
    return runtimeIdentity(slot)?.checkoutPath ?? null;
}
export function validatePaneIdentity(slot, currentPath, paneId) {
    if (!/^%\d+$/.test(paneId)) {
 ...
 */
export async function verifyPaneIdentity(slot, runShell = execShell) {
    ...
    try {
        ...
        `tmux display-message -t ${address} -p '#{pane_id}|#{pane_current_path}'`, { timeout: 3_000 });
        const direct = parsePaneIdentityLine(result.stdout.trimEnd().split(/\r?\n/, 1)[0] ?? "");
        if (direct) {
            try {
                const checkout = await runShell(`git -C ${shellEscape(direct.panePath)} rev-parse --show-toplevel`, { timeout: 3_000 });
        ...
        const listed = await runShell(`tmux list-panes -t ${address.slice(0, address.lastIndexOf("."))} -F '#{pane_id}|#{pane_current_path}'`, { timeout: 3_000 });
        const matches = [];
        ...
        for (const line of lines) {
            ...
            try {
                const checkout = await runShell(`git -C ${shellEscape(candidate.panePath)} rev-parse --show-toplevel`, { timeout: 3_000 });
                const validated = validatePaneIdentity(slot, checkout.stdout, candidate.paneId);
                if (validated.ok)
                    matches.push(validated.snapshot);
            }
            catch {
...
export function validatePaneLayout(paneAddresses, slotCount = 6) {
...
//# sourceMappingURL=paneIdentity.js.map