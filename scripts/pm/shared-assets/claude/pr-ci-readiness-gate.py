...
CANONICAL_CHANGE_SCOPE = Path("/Users/rajiv/.claude/scripts/ci/change_scope.py")
CANONICAL_REPO_ROOT = Path("/Users/rajiv/Downloads/projects/heydonna-app")
...
def change_scope_script() -> Path:
    candidates = [
        ...
        Path(__file__).resolve().parent / "ci" / "change_scope.py",
...
def exact_head_change_scope(pr: int, head: str, repo: str) -> dict[str, Any]:
    ...
    if not CANONICAL_REPO_ROOT.is_dir():
        raise FileNotFoundError("canonical repository root is unavailable")
    output = run(
        [
            "python3",
            str(classifier),
