#!/usr/bin/env python3
"""CTO-owned, exact-head label-gated CI admission boundary with main ancestry fencing.
...
def _origin_main(args: argparse.Namespace) -> str:
    checkout = str(Path(args.checkout).resolve())
    fetched = _run(["git", "-C", checkout, "fetch", "--no-tags", "origin", "main:refs/remotes/origin/main"])
    if fetched.returncode != 0:
        raise Refusal("main_object_unavailable")
    result = _run(["git", "-C", checkout, "rev-parse", "--verify", MAIN_REF])
    main = result.stdout.strip()
    if result.returncode != 0 or not HEAD_RE.fullmatch(main):
        raise Refusal("main_object_unavailable")
    return main


def _main_ancestry_fence(args: argparse.Namespace, *, expected_main: str | None = None, expected_labels: list[str] | None = None) -> str:
    main = _origin_main(args)
    pr = _live_pr(args)
    base = str(pr.get("baseRefOid") or "")
    if not HEAD_RE.fullmatch(base) or base != args.base:
        raise Refusal("main_base_drift")
    if expected_main is not None and main != expected_main:
        raise Refusal("main_advanced_before_effect")
    if base != main:
        raise Refusal("MAIN_INTEGRATION_REQUIRED")
    if expected_labels is not None and _labels(pr) != expected_labels:
        raise Refusal("labels_changed_before_effect")
    _checkout_head(args)
    result = _run(["git", "-C", str(Path(args.checkout).resolve()), "merge-base", "--is-ancestor", main, args.head])
    if result.returncode == 1:
        raise Refusal("MAIN_INTEGRATION_REQUIRED")
    if result.returncode != 0:
        raise Refusal("main_ancestry_unavailable")
    return main
...
def _live_pr(args: argparse.Namespace) -> dict[str, Any]:
    value = _gh_json(args, ["pr", "view", str(args.pr), "--repo", REPO, "--json", "number,headRefOid,baseRefOid,state,isDraft,mergeable,mergeStateStatus,headRefName,title,body,closingIssuesReferences,labels"], "pr_state")
    ...
    if not HEAD_RE.fullmatch(str(value.get("baseRefOid") or "")):
        raise Refusal("base_ref_unavailable")
...
def _load_effect_receipt(args: argparse.Namespace, expected_main: str) -> dict[str, Any] | None:
    ...
    if (
        ...
        or value.get("main_sha") != expected_main
...
def _effect(args: argparse.Namespace, before: list[str], expected_body_sha256: str, expected_main: str) -> dict[str, Any]:
    with _single_flight(args.pr):
        ...
        receipt = _load_effect_receipt(args, expected_main)
        if receipt is not None:
            if receipt["state"] == "completed":
                return {"status": "ALREADY_ADMITTED", "effect": "none", "partial": bool(receipt.get("partial")), "main_sha": expected_main}
        ...
        if target in current:
            main_sha = _main_ancestry_fence(args, expected_main=expected_main, expected_labels=current)
            return {"status": "ALREADY_ADMITTED", "effect": "none", "partial": bool(stale), "main_sha": main_sha}
        if len(stale) > 1:
            raise Refusal("concurrent_wave_detected")
        additions = ["pm-state:qa-passed-awaiting-ci", target]
        main_sha = _main_ancestry_fence(args, expected_main=expected_main, expected_labels=current)
        if hashlib.sha256(_issue_body(args).encode("utf-8")).hexdigest() != expected_body_sha256:
            raise Refusal("issue_body_changed_before_effect")
        _save_effect_receipt(args, "effect_started", before=before, issue_body_sha256=expected_body_sha256, main_sha=main_sha, base_ref_oid=args.base)
        ...
        if target not in after_post:
            _save_effect_receipt(args, "unresolved", observation="target_absent_after_post", main_sha=main_sha, base_ref_oid=args.base)
            raise Refusal("label_trigger_no_effect_no_retry")
        response_lost = post.returncode != 0
        if response_lost:
            _save_effect_receipt(args, "unresolved", observation="target_present_response_lost", main_sha=main_sha, base_ref_oid=args.base)
        ...
        for label in stale:
            ...
            if label in after_delete:
                _save_effect_receipt(args, "unresolved", observation="stale_label_delete_unconfirmed", main_sha=main_sha, base_ref_oid=args.base)
                raise Refusal("label_transition_partial_uncertain")
            if delete.returncode != 0:
                _save_effect_receipt(args, "unresolved", observation="stale_label_delete_response_lost", main_sha=main_sha, base_ref_oid=args.base)
                raise Refusal("label_transition_applied_response_lost")
        final = _readback_labels(args)
        if target not in final:
            _save_effect_receipt(args, "unresolved", observation="target_missing_final", main_sha=main_sha, base_ref_oid=args.base)
            raise Refusal("label_transition_uncertain")
        _save_effect_receipt(args, "completed", partial=False, main_sha=main_sha, base_ref_oid=args.base)
        return {"status": "ADMITTED", "effect": "one_ci_trigger_label_transition", "partial": False, "main_sha": main_sha}
...
def main() -> int:
    ...
    _checkout_head(args)
    pr = _live_pr(args)
    initial_main = _main_ancestry_fence(args)
    ...
    result = _effect(args, _labels(pr), body_sha256, initial_main)
    print(_json({"status": result["status"], "pr": args.pr, "issue": args.issue, "head": args.head, "base_ref_oid": args.base, "main_sha": result["main_sha"], "effect": result["effect"], "issue_body_sha256": body_sha256}))
    return 0
