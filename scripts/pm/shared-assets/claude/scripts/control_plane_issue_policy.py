...
# CTO 2026-09-10 (thread 1789047987.191339): control-plane classification is
# DEPRECATED for APP-OWNED CI/workflow/test tooling. A truthful app-CI defect
# issue for these surfaces (e.g. scripts/ci/rerun-on-infra-failure,
# scripts/ci/convex-start-env, and that class of app-owned CI/test/workflow
# tooling) must be fileable as a GitHub issue. This is an allowlist, not a
# broadening: it only carves out these named app-CI surfaces.
# Rajiv 2026-09-11 (thread 1789111520): "app ci is not control plane." The
# app-CI CI/E2E WORKFLOW FILES (.github/workflows/ci.yml, e2e*.yml — build/test
# job topology like the TypeScript+Python parallelization) are app CI, not the
...
_APP_CI_SURFACE_RE = re.compile(
    r"\b(?:"
    r"scripts/ci|"
    r"rerun[-_ ]on[-_ ]infra[-_ ]failure|"
    r"convex[-_ ]start[-_ ]env|"
    r"github/workflows|"
    r"ci\.yml|"
    r"e2e[-_a-z0-9]*\.yml"
    r")\b",
...
# Genuinely-forbidden non-app control-plane state-machine surfaces. These stay
# refused even inside an app-CI-allowlisted follow-up. This is the machine
# path/symbol subset of _INTERNAL_SURFACE_RE, deliberately EXCLUDING the bare
# "control-plane" prose term so that an app-CI issue whose only control-plane
# signal is a deprecation disclaimer ("control-plane is deprecated") is not
# forbidden by that prose alone.
# Rajiv 2026-09-11 (thread 1789111520): `github/workflows` is REMOVED from the
# genuine set — app-CI CI/E2E workflow files are app CI (now in the allowlist
# above), not the PR/slot state machine. A workflow-file issue that ALSO names
# a genuine state-machine surface below (slot dispatch, issue-admission,
# master-of-panes, etc.) still stays refused via that surface.
_GENUINE_CONTROL_PLANE_SURFACE_RE = re.compile(
    r"\b(?:"
    r"scripts/pm|"
    r"affected[-_ ]test[-_ ]plan(?:\.py)?|"
    r"pr[-_ ]ci[-_ ]readiness[-_ ]gate|"
...
def internal_control_plane_reason(title: str, body: str) -> str | None:
    ...
    haystack = f"{title}\n{followup_contract}"
    if INTERNAL_FOLLOWUP_MARKER in body and _INTERNAL_SURFACE_RE.search(haystack):
        # App-owned CI/workflow/test tooling is fileable (CTO 2026-09-10):
        # admit when the follow-up names an app-CI surface and does NOT also
        # name a genuine non-app control-plane state-machine surface. A mixed
        # issue that also targets a genuine control-plane surface stays refused.
        if _APP_CI_SURFACE_RE.search(haystack) and not _GENUINE_CONTROL_PLANE_SURFACE_RE.search(
            haystack
        ):
            return None
