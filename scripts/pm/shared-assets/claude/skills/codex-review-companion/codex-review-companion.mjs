...
// ---------- canonical terminal verification (obligation 18346, addendum) ----------

const TERMINAL_MARKER_BASENAME_RE = /^codex-app-(plan|code|qa)-review-(\d+)\.txt$/;

function terminalRefusal(code, detail) {
  return { ok: false, verdict: null, reason: `${code}${detail ? ` ${detail}` : ""}` };
}

function verifyMarkerTerminal(markerText, { head, number, reviewType } = {}) {
  const lines = String(markerText).split("\n");
  const field = (name) => {
    const line = lines.find((l) => l.startsWith(name + ":"));
    return line ? line.slice(name.length + 1).trim() : null;
  };
  const verdict = field("VERDICT");
  const companion = field("COMPANION_VERDICT");
  const final = field("FINAL_REVIEWER_VERDICT");
  if (!verdict || !companion || !final) {
    return terminalRefusal("TERMINAL_MISSING_VERDICT");
  }
  if (verdict !== companion || verdict !== final) {
    return terminalRefusal("TERMINAL_HEADER_MISMATCH", `${verdict}/${companion}/${final}`);
  }
  if (field("MARKER_PROVENANCE") !== "codex-review-companion") {
    return terminalRefusal("TERMINAL_NOT_CANONICAL_PROVENANCE");
  }
  if (!/^[0-9]{9,11}$/.test(field("TIMESTAMP") || "")) {
    return terminalRefusal("TERMINAL_BAD_TIMESTAMP");
  }
  if (number !== undefined && field("PR") !== `#${number}`) {
    return terminalRefusal("TERMINAL_PR_MISMATCH");
  }
  if (reviewType && field("TYPE") !== `${reviewType}-review`) {
    return terminalRefusal("TERMINAL_TYPE_MISMATCH");
  }
  if (head) {
    if (field("HEAD_SHA") !== head) {
      return terminalRefusal("TERMINAL_HEAD_MISMATCH");
    }
    const oid = field("headRefOid");
    if (oid && oid !== head) {
      return terminalRefusal("TERMINAL_HEAD_MISMATCH");
    }
  }
  if (/^APPROVE$|^CONFIRMED$|^VERIFIED$|^APPROVE_PENDING_CI$/.test(verdict)
      && lines.some((l) => l.includes("BLOCKER_STATUS: OPEN"))) {
    return terminalRefusal("TERMINAL_OPEN_BLOCKERS");
  }
  const findIdx = lines.findIndex((l) => l.startsWith("--- Findings ("));
  if (findIdx === -1) {
    return terminalRefusal("TERMINAL_MISSING_FINDINGS_SCAFFOLD");
  }
  const declared = (lines[findIdx].match(/--- Findings \((\d+)\) ---/) || [])[1];
  const outIdx = lines.findIndex((l) => l === "--- Review Output ---");
  const findingLines = lines.slice(findIdx + 1, outIdx === -1 ? undefined : outIdx)
    .filter((l) => /^[P][012]:/.test(l));
  if (declared === undefined || Number(declared) !== findingLines.length) {
    return terminalRefusal("TERMINAL_FINDINGS_COUNT_MISMATCH", `${declared}/${findingLines.length}`);
  }
  if (outIdx === -1) {
    return terminalRefusal("TERMINAL_MISSING_BODY");
  }
  ...
  return { ok: true, verdict, reason: null };
}

function resolveTerminalVerdict({ markerPath, head, number, reviewType } = {}) {
  // Exact-head and PR bindings are mandatory: an unbound APPROVE
  // compatibility path would let a genuine marker for another head/PR read
  // as this head's terminal. Fail closed before parsing when either is
  // absent (PR Reviews v2 REVISE, obligation 18346).
  if (number === undefined || number === null || !head) {
    return terminalRefusal("TERMINAL_BINDING_REQUIRED", "number and head are both required");
  }
  // Only the canonical writer location can produce a terminal. Agent-tool
  // subagent reports (jsonl transcripts, pasted text) are advisory evidence
  // only and MUST NOT satisfy a marker/exact-head gate, even when they
  // contain MARKER_* lines.
  const base = String(markerPath || "").split("/").pop() || "";
  const m = base.match(TERMINAL_MARKER_BASENAME_RE);
  let dir = String(markerPath || "");
  dir = dir.slice(0, dir.length - base.length).replace(/\/+$/, "") || "/";
  if (dir === "/private/tmp") dir = "/tmp";
  if (!m || dir !== "/tmp") {
    return terminalRefusal("TERMINAL_NOT_CANONICAL_PATH", String(markerPath || ""));
  }
  if (number !== undefined && Number(m[2]) !== Number(number)) {
    return terminalRefusal("TERMINAL_PR_MISMATCH", base);
  }
  let text;
  try {
    text = fs.readFileSync(String(markerPath), "utf8");
  } catch (e) {
    return terminalRefusal("TERMINAL_MARKER_ABSENT", base);
  }
  return verifyMarkerTerminal(text, { head, number, reviewType: reviewType || m[1] });
}
