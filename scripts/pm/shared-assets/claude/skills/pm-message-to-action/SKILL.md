...
| CI/E2E success terminal | `Skill(ci-success-reconciliation)` | An exact green CI+E2E terminal for a PR | Do not treat a generic green context or quoted success as current. On a fresh event with no sentinel, use the existing action-owned `--materialize --pr <PR>` boundary first; then use its normal `--claim --pr <PR>` path. Both steps must bind the live exact head and preserve the event/head/dedup evidence. Retain the existing CI terminal record, evidence, sentinel, and pending-intake bookkeeping. |
