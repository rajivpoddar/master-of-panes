...
#
# This sweep owns current PR motion. slot-dispatch-sweep owns fresh status:todo
# issue intake. Run this first so "no status:todo" cannot hide stale CI blockers,
...
PM_OPERATOR="${PM_OPERATOR:-/Users/rajiv/.claude/pm-operator/current/bin/pm-operator}"
...
data = {
    ...
    "required_command": "/Users/rajiv/.claude/pm-operator/current/bin/pm-operator reconcile-capacity",
    "prs": pr_nums,
    "slots": slots,
...
PYEOF
  ...
  mop_release="not-required"
  if [ "$should_release" = "yes" ]; then
    if release_output="$(
      "$PM_OPERATOR" release-slot \
        --slot "$slot" --expected-epoch "$expected_epoch" --reason "$reason" \
        --repository-id "$expected_repository" --issue "$expected_issue" --pr "$expected_pr" \
        --branch "$expected_branch" --head-sha "$expected_head" --mop-url "$MOP_HOST" 2>&1
    )"; then
      mop_release="released"
    else
      emit "PR_SLOT_RELEASE_FAILED PR#$pr slot:$slot issue=#${issue:-0} branch=${branch:-unknown} expected_epoch=$expected_epoch reason=$reason detail=$(printf '%s' "$release_output" | one_line_excerpt)"
      if [ -x "$PM_OPS" ]; then
        python3 "$PM_OPS" record \
          --source pr-state-sweep \
          --event pr_slot_release_failed \
          --target-type pr \
          --target-id "$pr" \
          --pr "$pr" \
          --issue "${issue:-0}" \
          ...
          --payload "detail=$(printf '%s' "$release_output" | one_line_excerpt)" >/dev/null 2>&1 || true
      fi
      return 1
    fi
  ...
  gh pr edit "$pr" --repo "$GH_REPO" --remove-label "slot:${slot}" >/dev/null 2>&1 || true
  [ -n "$issue" ] && gh issue edit "$issue" --repo "$GH_REPO" --remove-label "slot:${slot}" >/dev/null 2>&1 || true
  if [ -x "$PM_OPS" ]; then
    python3 "$PM_OPS" record \
      --source pr-state-sweep \
      --event pr_slot_label_released \
      --target-type pr \
      --target-id "$pr" \
      --pr "$pr" \
	      --issue "${issue:-0}" \
	      ...
	      --payload "branch=$branch" \
	      --payload "reason=$reason" \
	      --payload "mop_release=$mop_release" \
	      --payload "expected_epoch=${expected_epoch:-none}" >/dev/null 2>&1 || true
  fi
}
...
def ci_required_action_line(pr, reason, workflow_state, ci_run, e2e_run, issue, branch):
    ...
    if breaker:
        ...
        return (
            ...
            f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ ci\\ --ci-class\\ {command_quote(category)}"
    ...
    if workflow_state == "terminal_bad" and pr_requires_fresh_capture_before_ci(n):
        ...
        if local_capture_ok:
            ...
            return (
                ...
                f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ capture"
            )
    if workflow_state == "terminal_bad" and pressure["current_head_bad"] > 0:
        ...
        if ci_verdict_requires_rework(verdict):
            ...
            return (
                ...
                f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ ci\\ --ci-class\\ {command_quote(classification)}\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
            )
        ...
        # classes to block + assign-rework with the exact failed-run packet.
        return (
            ...
            f"command={rerun_wrapper} (at most one same-head retry, no sealed proof); if the wrapper refuses with a non-infra classification, run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ ci\\ --ci-class\\ current-head-failure\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
        )
...
def rework_packet_candidates(pr_number, slot):
    patterns = [
        f"/tmp/pm-transition-rework-{pr_number}-{slot}.md",
...
# review-PASS receipt (pm-transition pm-review-done) for the exact head. When
...
def pm_review_done_receipt_line(pr_number, head, issue, branch, prefix):
    ...
    if blocked.get("class"):
        ...
        print(
            ...
            f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ pm-review-done\\ --pr\\ {pr_number}"
        )
    return True
...
def dependency_unblocked_command(n, dep_reason=""):
    proof = ""
    m = re.search(r"\bproof=([^ ]+)", dep_reason or "")
    if m:
        proof = m.group(1)
    proof_arg = f"\\ --proof\\ {command_quote(proof)}" if proof else ""
    return f"resolve_the_GitHub_dependency_for_PR_{n}{proof_arg} then /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
...
for pr in sorted(prs, key=lambda p: int(p.get("number") or 0), reverse=True):
            ...
            if behind_by is not None and behind_by >= main_behind_threshold:
                if mergeable_and_green:
                    ...
                    next_step = (
                        ...
                        if owner_slot
                        else "create_current_head_merge-main_packet_then_assign-rework_before_fresh_todo"
                    )
                    print(
    ...
    if has_label(pr, "pm-blocked:dependency"):
        ...
        print(
            ...
            f"issue=#{issue or 'unknown'} branch={branch} head={head[:10]} "
            f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ dependency\\ --dependency-pr\\ '<PR_OR_ISSUE>'"
        )
        continue
    ...
    if has_label(pr, "pm-blocked:capture"):
        ...
        if not active_slot and fuzzy_slot:
            ...
            if emit_incomplete_owner(fuzzy_slot):
                ...
                f"assignment_epoch={fuzzy_slot.get('assignment_epoch')} {owner_tokens(fuzzy_slot)} mop={slot_entry_summary(fuzzy_slot)} command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
                )
            continue
        if active_slot and state == "pm-state:blocked-rework":
            ...
            if rearm:
                ...
                print(
                    ...
                    f"command=merge_authorized_main_release_slot_then_run_pm-transition_capture-remote-dispatch_on_new_exact_head"
                )
                continue
            ...
            if capture_rework_by_pr.get(int(n)):
                ...
                print(
                    ...
                    f"mop={slot_entry_summary(active_slot)} "
                    f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
                )
                continue
        ...
        if active_slot:
            ...
            print(
                ...
                f"state={state or 'no_state'} issue=#{issue or 'unknown'} branch={branch} head={head[:10]} "
                f"mop={slot_entry_summary(active_slot)} "
                f"remediation=release_slot_{slot}_with_the_complete_authoritative_tuple"
            )
            continue
        if slots_for_pr and state != "pm-state:pm-review-pending":
            ...
            print(
                ...
                f"state={state or 'no_state'} issue=#{issue or 'unknown'} branch={branch} head={head[:10]} "
                f"command=gh\\ pr\\ edit\\ {n}\\ --remove-label\\ slot:{slot}\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
            )
            continue
    ...
    if state == "pm-state:blocked-rework" and merge_state in {"BEHIND", "DIRTY"} and not has_label(pr, "pm-blocked:rebase"):
        print(
            ...
            f"head={head[:10]} command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ rebase"
        )
        continue
    ...
    if state == "merge-ready" and has_label(pr, "pm-state:merge-ready"):
        print(
            ...
            f"issue=#{issue or 'unknown'} branch={branch} head={head[:10]} "
            f"command=/Users/rajiv/.claude/scripts/pm-state-replace.sh\\ {n}\\ merge-ready"
        )
        continue
    ...
    if state == "merge-ready" and slots_for_pr:
        for slot in slots_for_pr:
            print(
                ...
                f"issue=#{issue or 'unknown'} branch={branch} remediation=release_slot_{slot}_with_the_complete_authoritative_tuple"
            )
        continue
    ...
    if state == "pm-state:qa-passed-awaiting-ci":
        ...
        if active_slot:
            ...
            print(
                ...
                f"remediation=release_slot_{slot}_with_the_complete_authoritative_tuple\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
            )
            continue
        if slots_for_pr:
            slot = slots_for_pr[0]
            print(
                ...
                f"issue=#{issue or 'unknown'} branch={branch} head={head[:10]} "
                f"command=gh\\ pr\\ edit\\ {n}\\ --repo\\ {gh_repo}\\ --remove-label\\ slot:{slot}\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ reconcile-capacity"
            )
            continue
    ...
    if state == "pm-state:qa-passed-awaiting-ci" and merge_state in {"BEHIND", "DIRTY"}:
        print(
            ...
            f"mergeStateStatus={merge_state} issue=#{issue or 'unknown'} branch={branch} "
            f"head={str(pr.get('headRefOid') or '')[:10]} command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ rebase"
        )
        continue
    ...
    if state == "pm-state:qa-passed-awaiting-ci" and not has_label(pr, "pm-blocked:ci"):
        ...
        if workflow_state == "terminal_green" and merge_state == "CLEAN" and not draft:
            ...
            print(
                ...
                f"command=run_pm_readiness_contract_then_validate-ready-proof_then_pm-transition_merge-ready"
            )
            continue
        if workflow_state == "missing_or_skipped" and merge_state == "CLEAN" and not draft:
            if has_label(pr, "pm-blocked:dependency"):
    ...
    if state == "pm-state:pm-review-pending":
        if has_label(pr, "pm-blocked:capture"):
            ...
            if local_capture_ok and pending_event:
                print(
                    ...
                    f"head={str(pr.get('headRefOid') or '')[:10]} "
                    f"remediation=consume_the_exact_local_capture_result\\ &&\\ gh\\ pr\\ edit\\ {n}\\ --remove-label\\ pm-blocked:capture\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ slot-ready\\ --event\\ {pending_event}"
                )
                continue
            if local_capture_ok:
                print(
                    ...
                    f"local_capture_proof={local_capture_proof} issue=#{issue or 'unknown'} branch={branch} "
                    f"head={str(pr.get('headRefOid') or '')[:10]} "
                    f"remediation=consume_the_exact_local_capture_result\\ &&\\ gh\\ pr\\ edit\\ {n}\\ --remove-label\\ pm-blocked:capture\\ &&\\ /Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ pm-review-done\\ --pr\\ {n}"
                )
                continue
        ...
        if marker_status == "blocked":
            print(
                ...
                f"head={str(pr.get('headRefOid') or '')[:10]} "
                f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ block-pr\\ --pr\\ {n}\\ --reason\\ pm-gate"
            )
            continue
        ...
        if pm_review_pending_is_stale(meta, str(pr.get("headRefOid") or "")):
            ...
            print(
                ...
                f"issue=#{issue or 'unknown'} branch={branch} head={str(pr.get('headRefOid') or '')[:10]} "
                f"command=run_pm_claude_code_review_then_pm-transition_pm-review-done_or_rework"
            )
            continue
        continue
            ...
            if idle:
                ...
                if pending_delivery:
                    print(
                        ...
                        "command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator reconcile-capacity"
                    )
                else:
    ...
    if draft and not state and not blockers and recently_updated(pr, 96):
        ...
        print(
            ...
            f"command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ pm-review\\ --pr\\ {n}\\ --scope\\ phase-a"
        )
        continue
    ...
    if not state and not draft and recently_updated(pr, 96):
        ...
        print(
            ...
            f"branch={branch} checks={check_state} command=/Users/rajiv/.claude/pm-operator/current/bin/pm-operator\\ pm-review\\ --pr\\ {n}\\ --scope\\ phase-a"
        )
    ...
    case "$line" in
      ...
      PR_REWORK_PACKET_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Create the exact rework packet, then run pm-transition record-rework-packet --pr $pr --packet FILE. Existing PR rework must be assigned before fresh status:todo work." \
      ...
      PR_READY_PROMOTION_REQUIRED*)
        ...
        if [ "$DRY_RUN" = "1" ] || [ "$ready_gate" != "pass" ]; then
          ...
          upsert_obligation \
            ...
            --action "Run pm-readiness-contract; if PASS, write /tmp/pm-state-promotion-proof-$pr-merge-ready.ok with exact current headRefOid, then run pm-operator validate-ready-proof --pr $pr and pm-operator merge-ready --pr $pr before dispatching fresh issue work. If BLOCKED, apply the blocker state instead of dispatching new work." \
        ...
        if [ -z "$live_head" ] || [ "$live_head" != "$head" ]; then
          ...
          transition_output="$("$PM_OPERATOR" validate-ready-proof --repository-id "$GH_REPO" --pr "$pr" --proof "/tmp/pm-state-promotion-proof-${pr}-merge-ready.ok" 2>&1)"
          transition_rc=$?
          if [ "$transition_rc" -eq 0 ] \
            && printf '%s\n' "$transition_output" | grep -q '"decision": "READY_PROOF_VALIDATED"'; then
            merge_output="$("$PM_OPERATOR" merge-ready --repository-id "$GH_REPO" --pr "$pr" --proof "/tmp/pm-state-promotion-proof-${pr}-merge-ready.ok" 2>&1)"
            merge_rc=$?
          else
            merge_rc="$transition_rc"
            merge_output="$transition_output"
          fi
          if [ "$merge_rc" -eq 0 ] \
            && printf '%s\n' "$merge_output" | grep -q '"decision": "MERGE_READY_COMMITTED"' \
            && printf '%s\n' "$merge_output" | grep -q "\"head_sha\": \"${head}\""; then
      ...
      PR_PM_REVIEW_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Run/finish PM Claude phase-A review only. If review is in-flight, set /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review --pr $pr --scope phase-a; if PASS, run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done. Merge-ready uses pm-readiness-contract, not pm-review --scope merge-ready." \
      ...
      PR_PM_REVIEW_TERMINAL_BLOCKED_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Do not launch another same-head review and do not re-run pm-review-done until the blocker clears. Execute the typed wake: ${wake:-see sweep row}. After the wake is satisfied, resume /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done --pr $pr (idempotent resume from the exact-head terminal receipt; no duplicate terminal event/obligation/label event/CI wave)." \
      ...
      PR_PM_REVIEW_TERMINAL_RECEIPT_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Resume /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done --pr $pr from the head-bound terminal receipt. The resume is idempotent: no second reviewer, no duplicate terminal event, obligation, label event, or CI wave; head drift invalidates reuse." \
      ...
      PR_PM_REVIEW_AFFECTED_TEST_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Affected-test proof is retired (Rajiv thread 1786811168.455449). Run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done --pr $pr at the exact head to release ownership and move to qa-passed-awaiting-ci; do not rerun PM review." \
      ...
      PR_PM_REVIEW_CAPTURE_REQUIRED*|PR_PM_REVIEW_CAPTURE_BYPASS_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --title "PR #$pr PM Claude review requires remote capture before CI" \
	          --action "Do not run pm-review-done until the exact-head remote capture is green. Dispatch through pm-transition capture-remote-dispatch --pr $pr and consume success only with capture-remote-pass --pr $pr --run <RUN_ID>. Local capture is diagnostic-only and never satisfies capture readiness." \
      ...
      PR_PM_REVIEW_CAPTURE_COMPLETE_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --title "PR #$pr remote capture complete; consume PM Claude review" \
	          --action "Exact-head remote capture is authoritative. Consume it with pm-transition capture-remote-pass --pr $pr --run <RUN_ID>; that creates the post-capture slot local-repro requirement (next=ci-local-preflight-repro) bound to the exact head + capture run. Do NOT run pm-review-done or route readiness/CI. The slot runs the canonical local repro and records the raw classification (no sealed envelope) through pm-transition ci-local-preflight-pass; CTO fires the next exact-head label-gated CI+E2E wave (cto_ci_wave_required)." \
      ...
      PR_CAPTURE_LOCAL_PASS_CI_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "The local diagnostic proof was recorded; it does not satisfy capture readiness. Dispatch the authoritative remote capture through pm-transition capture-remote-dispatch --pr $pr and consume success only with capture-remote-pass --pr $pr --run <RUN_ID>." \
      ...
      PR_CAPTURE_COMPLETE_SLOT_READY_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Remove pm-blocked:capture and run the emitted pm-transition slot-ready --event command. The transition must release matching slot ownership and assert no slot:N remains before qa-passed-awaiting-ci is applied. Skipped/dummy contexts do not count as readiness." \
      ...
      PR_CAPTURE_COMPLETE_PM_REVIEW_DONE_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Remove pm-blocked:capture and run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done --pr $pr. The transition must release matching slot ownership and assert no slot:N remains before qa-passed-awaiting-ci starts label-gated CI after local capture proof. Skipped/dummy contexts do not count as readiness." \
      ...
      PR_CAPTURE_BEFORE_CI_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --title "PR #$pr prompt/LLM-path CI failure requires capture before rerun" \
	          --action "Run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator block-pr --pr $pr --reason capture. Run slot-local capture proof on the exact current PR head, consume it, then trigger label-gated CI directly. Do not dispatch generic local CI rework or rerun CI until local capture proof exists on the current head." \
          --blocker "capture_before_ci" \
          --evidence "$line"
        ;;
      ...
      PR_CI_CAPPED_RERUN_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Run at most one same-head rerun of the failed CI/E2E run through /Users/rajiv/Downloads/projects/heydonna-app/.claude/scripts/ci/rerun-after-local-proof.sh --pr $pr --run <failed-run> (no sealed proof required; Rajiv 1786812200.371389). The wrapper enforces the one-retry cap and refuses product classes; on a non-infra refusal, block-pr --reason ci --ci-class current-head-failure and reconcile-capacity (assign-rework with the exact failed-run packet). Never toggle CI labels on the same head." \
          --blocker "ci_rerun_after_preflight" \
          --evidence "$line"
        ;;
      ...
      PR_DEPENDENCY_UNBLOCKED_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Resolve the GitHub dependency for PR $pr using the emitted proof, then run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator reconcile-capacity. The dependency-block marker no longer justifies a passive wait." \
      ...
      PR_DEPENDENCY_WEDGE_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Do not treat pm-blocked:dependency as a passive wait until the blocker is machine-readable. Run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator block-pr --pr $pr --reason dependency --dependency-pr <PR_OR_ISSUE> to write current-head proof, or remove pm-blocked:dependency and resume capture/CI. Current dependency label age_min=${age:-unknown}." \
      ...
      PR_EXTERNAL_WAIT_SLOT_CONFLICT_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --action "Decide which state is true. If Rajiv/product answered, resolve the current branch/head/expected epoch/repository/handoff and use the complete claim_slot assign-rework tuple for slot $slot; successful delivery clears pm-blocked:product and resolves product_decision_wait. If still waiting, run pm-transition block-pr --pr $pr --reason product to release the slot." \
      ...
      PR_PRODUCT_DECISION_WAITING*)
        ...
        upsert_obligation \
          ...
          --action "Do not treat pm-blocked:product as a quiet parked state without proof. PM must capture or post the exact product question, PM's recommended default, and Rajiv/product response. Once answered, remove pm-blocked:product and rerun pr-state-sweep/reconcile-capacity; until then this remains current PR work ahead of fresh issue dispatch." \
      ...
      PR_DRAFT_ORPHAN_REVIEW_REQUIRED*|PR_STATE_LABEL_REQUIRED*|PR_STATE_RECONCILE_REQUIRED*|PR_CI_CLASSIFICATION_REQUIRED*|PR_CI_DEPENDENCY_BLOCKED_REQUIRED*|PR_DEPENDENCY_UNBLOCKED_REQUIRED*|PR_READY_BEFORE_CI_REQUIRED*|PR_REBASE_REQUIRED*)
        ...
        upsert_obligation \
          ...
          --title "PR #$pr needs PM state reconciliation" \
	          --action "Apply the named PR transition before dispatching fresh issue work. For PR_CI_DEPENDENCY_BLOCKED_REQUIRED, run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator block-pr --pr $pr --reason dependency --dependency-pr <PR_OR_ISSUE> so the visible pm-blocked:dependency marker and payload exist, then wait until the dependency merges/closes before rerunning CI. For PR_READY_BEFORE_CI_REQUIRED, mark the PR ready first, then remove/re-add the CI-trigger label only if required workflows did not start on the current head." \
      ...
      PR_REVIEW_CIRCUIT_BREAKER_REQUIRED*)
        ...
        ;;
          ...
          --action "Affected-test verification is retired (Rajiv thread 1786811168.455449). Run /Users/rajiv/.claude/pm-operator/current/bin/pm-operator pm-review-done --pr $pr at the exact head so the current-head Phase-A PASS marker binds CI start; no affected-test packet or verification-only assignment is required." \
