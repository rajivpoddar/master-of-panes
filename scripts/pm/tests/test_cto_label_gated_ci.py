...
if args[:1] == ["fetch"]:
    ...
    if log:
        pathlib.Path(log).write_text(" ".join(args) + "\\n", encoding="utf-8")
    count_path = pathlib.Path(sys.argv[2], ".fetch-count")
    count = int(count_path.read_text()) if count_path.exists() else 0
    count_path.write_text(str(count + 1))
    mutation = os.environ.get("FAKE_FETCH_MUTATE")
    if mutation and count >= 1:
        state_path = pathlib.Path(os.environ["FAKE_STATE"])
        state = __import__("json").loads(state_path.read_text())
        if mutation == "labels":
            state["labels"].append("pm-blocked:codex")
        elif mutation == "head":
            state["head_ref_oid"] = "c" * 40
        elif mutation == "base":
            state["base_ref_oid"] = "c" * 40
        state_path.write_text(__import__("json").dumps(state))
    raise SystemExit(int(os.environ.get("FAKE_FETCH_RC", "0")))
if args[:2] == ["rev-parse", "--verify"]:
    if args[2] == "HEAD":
        print(pathlib.Path(sys.argv[2], ".git-head").read_text().strip())
    else:
        sequence = os.environ.get("FAKE_MAIN_SEQUENCE")
        if sequence:
            path = pathlib.Path(sys.argv[2], ".main-count")
            count = int(path.read_text()) if path.exists() else 0
            values = sequence.split(",")
            print(values[min(count, len(values) - 1)])
            path.write_text(str(count + 1))
        else:
            print(os.environ.get("FAKE_MAIN", {BASE!r}))
    raise SystemExit(0)
if args[:2] == ["merge-base", "--is-ancestor"]:
    raise SystemExit(int(os.environ.get("FAKE_ANCESTOR_RC", "0")))
raise SystemExit(2)
''', encoding="utf-8")
        git.chmod(0o755)
    ...
    def tearDown(self) -> None:
        ...
        for name in ("FAKE_ANCESTOR_RC", "FAKE_FETCH_RC", "FAKE_MAIN_SEQUENCE", "FAKE_MAIN", "FAKE_FETCH_MUTATE", "FAKE_GIT_LOG", "FAKE_STATE"):
...
if args[:2] == ["pr", "view"]:
 print(json.dumps({{"number":7622,"headRefOid":s.get("head_ref_oid", {HEAD!r}),"baseRefOid":s.get("base_ref_oid", {BASE!r}),"state":"OPEN","isDraft":False,"mergeable":"MERGEABLE","mergeStateStatus":"CLEAN","headRefName":"feature","closingIssuesReferences":[{{"number":s["linked_issue"]}}],"labels":[{{"name":x}} for x in s["labels"]]}}))
...
'''
    ...
    def _main(self, **extra):
        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()) as err:
            try:
                ...
                initial_main = self.module._main_ancestry_fence(args)
                ...
                result = self.module._effect(args, self.module._labels(pr), sha, initial_main)
    ...
    def test_main_ancestry_fence_refuses_behind_main_before_effect(self) -> None:
        os.environ["FAKE_ANCESTOR_RC"] = "1"
        code, _, error = self._main()
        self.assertEqual(code, 13)
        self.assertIn("MAIN_INTEGRATION_REQUIRED", error)
        self.assertEqual(self._state()["post_calls"], 0)

    def test_main_object_missing_refuses_without_effect(self) -> None:
        os.environ["FAKE_FETCH_RC"] = "1"
        code, _, error = self._main()
        self.assertEqual(code, 13)
        self.assertIn("main_object_unavailable", error)
        self.assertEqual(self._state()["post_calls"], 0)

    def test_main_advance_between_fences_refuses_without_effect(self) -> None:
        os.environ["FAKE_MAIN_SEQUENCE"] = f"{BASE},{'c' * 40}"
        code, _, error = self._main()
        self.assertEqual(code, 13)
        self.assertIn("main_advanced_before_effect", error)
        self.assertEqual(self._state()["post_calls"], 0)

    def test_final_fetch_revalidates_head_base_and_labels_before_post(self) -> None:
        for mutation, reason in (("head", "head_drift"), ("base", "main_base_drift"), ("labels", "labels_changed_before_effect")):
            with self.subTest(mutation=mutation):
                data = self._state()
                data["labels"] = ["unrelated", "ci-head:old"]
                data["head_ref_oid"] = HEAD
                data["base_ref_oid"] = BASE
                data["post_calls"] = 0
                self.state.write_text(json.dumps(data))
                (self.checkout / ".fetch-count").unlink(missing_ok=True)
                os.environ["FAKE_FETCH_MUTATE"] = mutation
                code, _, error = self._main()
                self.assertEqual(code, 13)
                self.assertIn(reason, error)
                self.assertEqual(self._state()["post_calls"], 0)
                os.environ.pop("FAKE_FETCH_MUTATE", None)

    def test_fetch_updates_explicit_tracking_ref_for_restricted_refspec(self) -> None:
        log = self.dir / "git.log"
        os.environ["FAKE_GIT_LOG"] = str(log)
        code, _, error = self._main()
        self.assertEqual(code, 0, error)
        self.assertIn("main:refs/remotes/origin/main", log.read_text(encoding="utf-8"))

    def test_admission_receipt_binds_main_sha(self) -> None:
        code, result, error = self._main()
        self.assertEqual(code, 0, error)
        self.assertEqual(result["main_sha"], BASE)
        receipt = json.loads(self.module._receipt_path(self._args()).read_text())
        self.assertEqual(receipt["main_sha"], BASE)
    ...
    def test_replay_and_concurrency_are_single_flight(self) -> None:
        ...
        def worker() -> None:
            try:
                results.append(("ok", self.module._effect(args, before, hashlib.sha256(self.issue_body.encode()).hexdigest(), BASE)))
            except Exception as exc:
    ...
    def test_issue_body_drift_after_proof_is_zero_effect(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "issue_body_changed_before_effect"):
            self.module._effect(args, self.module._labels(pr), body_sha, BASE)
        self.assertEqual(self._state()["post_calls"], 0)
