...
const baseRequest = (): Family2ReleaseEffectRequest => ({ base_url: "http://mop", slot: 4, effect_id: "effect-7525", expected_epoch: 581, expected_tuple: T, intended_main_head: "a".repeat(40) });
function fakeTransport(): { fetch: Family2ReleaseFetch; posts: number[]; calls: Array<{ url: string; init?: RequestInit }>; reuse: (value: Record<string, unknown>) => void } {
  ...
  const receipts = new Map<string, Record<string, unknown>>(); const posts: number[] = []; const calls: Array<{ url: string; init?: RequestInit }> = [];
  const fetch: Family2ReleaseFetch = async (input, init) => {
    ...
    if (url.pathname.endsWith("/release") && init?.method === "POST") { const body = JSON.parse(String(init.body)); if (!body.effect_id || !body.expected_repository_id || body.expected_session_id !== undefined) return { ok: false, status: 400, json: async () => ({ success: false, error: "session_id is not an ownership field" }) } as any; posts.push(1); current = { slot: 4, occupied: false, idle: true, active_turn_state: "inactive", assignment_epoch: 582, session_id: null }; receipts.set(body.effect_id, { effect_id: body.effect_id, request_digest: body.request_digest, slot: 4, expected_epoch: 581, released_epoch: 582, expected_tuple: T, intended_main_head: body.intended_main_head }); return { ok: true, status: 200, json: async () => ({ success: true, effect_id: body.effect_id, request_digest: body.request_digest }) } as any; }
    return { ok: true, status: 200, json: async () => current } as any;
  };
  return { fetch, posts, calls, reuse: (value) => { current = value; } };
}
