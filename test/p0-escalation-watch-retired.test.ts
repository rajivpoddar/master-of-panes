...
test("manual diagnostics remain read-only and heartbeat ownership is readable", async () => {
  await withPmOpsDb(async (pmOpsDb) => {
    ...
    assert.equal(heartbeat.command.includes("canonical read-only Sakshi producer preflight"), true);
    assert.equal(heartbeat.command.includes("nonzero producer result"), true);
