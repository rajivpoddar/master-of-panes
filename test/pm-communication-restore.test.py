...
class PMCommunicationRestoreTests(unittest.TestCase):
    ...
    def test_skill_contracts_are_short_and_use_existing_boundaries(self) -> None:
        ...
        self.assertIn("wake signal", processing)
        ...
        self.assertIn("preserve the current slot,\nmake no release POST, and return", processing)
        self.assertIn("PM_NUDGE_RECONCILIATION_PROCESSED reason=current_wait_not_due", processing)
        self.assertIn("This is a processed reconciliation result, never `STALE` or `IGNORED`", processing)
        self.assertIn("POST http://127.0.0.1:<MOP_PORT>/slots/{slot}/release", processing)
        self.assertIn("return its typed current-state blocker", processing)
        self.assertIn("occupied=false", processing)
        self.assertIn("execute the existing direct-assign\ncontract once", processing)
        self.assertIn("PM_ASSIGNMENT_BLOCKED reason=current_state_mismatch", processing)
        self.assertIn("Make exactly one request to the existing native boundary", direct_assign)
        self.assertIn("invoke the existing direct release boundary exactly once", direct_release)
