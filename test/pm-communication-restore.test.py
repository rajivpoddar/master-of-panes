...
class PMCommunicationRestoreTests(unittest.TestCase):
    ...
    def test_skill_contracts_are_short_and_use_existing_boundaries(self) -> None:
        ...
        self.assertIn("wake signal", wait)
        self.assertIn("Never classify a PM wait or free-slot notice as", wait)
        self.assertIn("STALE", wait)
        self.assertIn("IGNORED", wait)
        self.assertIn("re-read the named slot and current Ready Pool", wait)
        for drift_field in ("issue, PR", "epoch, owner", "wait age", "assignment", "free/occupied"):
            self.assertIn(drift_field, wait)
        self.assertIn("POST http://127.0.0.1:<MOP_PORT>/slots/{slot}/release", processing)
        self.assertIn("PM_NUDGE_RECONCILIATION_BLOCKED reason=current_state_mismatch", processing)
        self.assertIn("construct/relay the required release message", processing)
        self.assertIn("fixed priority: `repro`, then `rework`, then `new_issue`", processing)
        ...
        for drift_field in ("issue, PR", "epoch, owner", "wait age", "assignment", "free/occupied"):
            self.assertIn(drift_field, processing)
